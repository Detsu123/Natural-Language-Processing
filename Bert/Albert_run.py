# albert_cv_top10_40runs_light.py
import os, json
import numpy as np
import pandas as pd
...
X = np.load(os.path.join(ART_DIR, "albert_cls.npy")).astype(np.float32)
y = np.load(os.path.join(ART_DIR, "albert_labels.npy"))

OUT_DIR = os.path.join(ART_DIR, "albert_cv_top10_40runs")
...
df.to_csv(os.path.join(OUT_DIR, "albert_40_results.csv"), index=False)

print("✅ ALBERT DONE")
print("Saved →", os.path.join(OUT_DIR, "albert_40_results.csv"))
