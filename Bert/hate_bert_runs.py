# hatebert_cv_top10_40runs_light.py
import os, json
import numpy as np
import pandas as pd
...
X = np.load(os.path.join(ART_DIR, "hatebert_cls.npy")).astype(np.float32)
y = np.load(os.path.join(ART_DIR, "hatebert_labels.npy"))

OUT_DIR = os.path.join(ART_DIR, "hatebert_cv_top10_40runs")
...
df.to_csv(os.path.join(OUT_DIR, "hatebert_40_results.csv"), index=False)

print("✅ HATEBERT DONE")
print("Saved →", os.path.join(OUT_DIR, "hatebert_40_results.csv"))
